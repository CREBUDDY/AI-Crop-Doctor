...
import { AlertTriangle, CheckCircle2, ShieldCheck, Droplets, MapPin, ChevronLeft, Info, Calendar, Sprout, ArrowRight } from "lucide-react";
...
export function ReportPage() {
  ...
  if (error || !report) {
    return (
      <div className="container mx-auto p-8 text-center text-red-500 mt-20">
        ...
        <Link to="/dashboard" className="text-emerald-600 mt-4 inline-block hover:underline">{t('report.error.back')}</Link>
      </div>
    );
  }

  const primaryDisease = report.detected_diseases && report.detected_diseases.length > 0 ? report.detected_diseases[0] : null;
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-8 pb-24 max-w-5xl">
      ...
      {/* Main Health Score Card (Premium Redesign) */}
      <motion.div
        ...
        <div className="p-8 md:p-10 flex flex-col md:flex-row items-center gap-10 relative z-10">
          ...
          <div className="relative flex h-40 w-40 shrink-0 items-center justify-center rounded-full bg-white shadow-lg border border-gray-50">
            ...
            <div className="text-center">
              ...
              <span className="block text-sm font-bold text-gray-400 mt-1 uppercase tracking-widest">{t('report.score')}</span>
            </div>
          </div>

          <div className="text-center md:text-left flex-1">
            <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-4 border ${
              report.overall_severity === 'critical' ? 'bg-rose-100 text-rose-700 border-rose-200' :
              report.overall_severity === 'high' ? 'bg-orange-100 text-orange-700 border-orange-200' :
              report.overall_severity === 'moderate' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' :
              'bg-emerald-100 text-emerald-700 border-emerald-200'
            }`}>
              {report.overall_severity === 'none' ? `✅ ${t('report.status.healthy')}` : `🔴 ${report.overall_severity || 'Unknown'} ${t('report.status.severity')}`}
            </span>
        ...
        <div className="md:col-span-5 space-y-8">
          ...
          <Card className="border-0 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl bg-white">
            ...
            <CardContent className="pt-6 space-y-6">
              {report.detected_diseases?.map((d: any, i: number) => (
                <div key={`disease-${i}`}>
              ...
              {report.detected_pests?.map((p: any, i: number) => (
                <div key={`pest-${i}`}>
                  <div className="flex justify-between items-end mb-2">
              ...
              {!report.detected_diseases?.length && !report.detected_pests?.length && (
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between p-4 bg-primary-50 border border-primary-100 rounded-2xl">
                <div>
                  ...
                  <p className="text-2xl font-black text-primary-900">{t('report.recovery.days')}</p>
                </div>
          ...
          <Card className="border-primary-200 bg-gradient-to-b from-primary-50/50 to-white shadow-lg shadow-primary-500/5 rounded-3xl overflow-hidden h-full">
            ...
            <CardContent className="pt-8 space-y-8">
              ...
              <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100 relative overflow-hidden group hover:border-primary-300 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-4 pl-4 mb-6">
                  <div className="rounded-xl bg-gray-50 p-4 border border-gray-100">
                    ...
                    <div className="font-black text-gray-900 text-lg">600 g</div>
                  </div>
                  <div className="rounded-xl bg-gray-50 p-4 border border-gray-100">
                    <div className="text-sm text-gray-500 font-medium mb-1 flex items-center gap-1.5">
                      <Droplets className="w-4 h-4 text-gray-400"/> {t('report.plan.medicine.water')}
                    </div>
                    <div className="font-black text-gray-900 text-lg">200 Liters</div>
                  </div>
                </div>
                ...
                <div className="pl-4">
                  <div className="bg-amber-50 text-amber-900 p-4 rounded-xl border border-amber-200 flex gap-3 shadow-inner">
                    ...
                    <p className="text-sm leading-relaxed font-medium">
                      <span className="font-bold">{t('report.plan.medicine.important')}</span> {t('report.plan.medicine.warning')}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 flex flex-col sm:flex-row gap-4">
                ...
                <Button variant="outline" size="lg" className="w-full sm:w-auto h-14 px-8 bg-white border-gray-200 text-gray-700 font-bold rounded-xl">
